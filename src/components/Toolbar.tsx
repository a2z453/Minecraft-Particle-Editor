...
import { createCircleShape, createSphereShape, createSpiralShape, createStarShape } from '../utils/shapeGenerators';
...
export default function Toolbar() {
  ...
  const [filterColorable, setFilterColorable] = useState(false);

  const filteredParticles = Object.keys(PARTICLE_DATA).filter((type) =>
    type.includes(search.toLowerCase()) && (!filterColorable || PARTICLE_DATA[type].supportsColor)
  );
  ...
  return (
    <div className="space-y-4">
      ...
      <label className="flex items-center">
        <input
          type="checkbox"
          checked={filterColorable}
          onChange={(e) => setFilterColorable(e.target.checked)}
        />
        <span className="ml-2">Colorable only</span>
      </label>
      <select
        onChange={(e) => {
          const type = e.target.value;
          if (type) {
            addEmitter({ type, position: [0, 1, 0], rotation: [0, 0, 0], scale: [1, 1, 1], properties: PARTICLE_DATA[type] });
          }
        }}
        className="w-full p-2 border rounded"
      >
        ...
        {particleCategories.map((category) => (
          <optgroup key={category.category} label={category.category}>
            {category.particles.filter((type) => filteredParticles.includes(type)).map((type) => (
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        <button onClick={() => addEmitter(createStarShape())} className="p-2 bg-white rounded shadow">Star</button>
      </div>
    </div>
  );
}